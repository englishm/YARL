...
from xml.etree.ElementTree import Element, ElementTree  # this should be all we need
...
class ADXFile:
    ...
    This class provides 3 functions that can be called to add QSO information to a .adx file. The file that will be
    written to is passed in as a path during instantiation. The class checks if the path is OK to use, and if it is,
    you can then use
    """

    def __init__(self, file):
        """
        Create a new ADXFile that can be used to export QSOs into ADIF (specifically, ADX files).

        :param file: Location to create file at.
        :return: None
        """
        # we're going to throw an error if the location to export to already exists. It shouldn't, if the GUI is done
        # right, but we're going to anyway for posterity.
        ...
        self._root = self._tree.getroot()  # use _add_tag() to add a subtag to it in a nicer format
        self._in_records = False  # we are going to check these two variables to see if we are currently writing records
        self._records = None  # and what the main records tag is
    ...
    def write_header(self, _adif_ver="3.0.8", _app_name="YARL"):
        ...
        :param _adif_ver: ADIF version. Default and currently supported version is "3.0.8"
        :param _app_name: Name of the application. Default "YARL"
        :return: None
        :raise: ADIFExportError if called twice.
        """
        if self._header_written:
            raise ADIFExportError("Attempted adding header to file which already had a header")
        if self._in_records:
    ...
    def write_record(self, data: dict):
        ...
        if not self._in_records and self._records is None:  # great, we're starting to write our records now!
